...
def write_sql_database(df, table_name, if_exists='replace'):
    user = getpass.getuser()
    path = os.path.join('app_config','my.cnf')
...
def write_csv_sql(df, name, path="", sql=True, csv=True, if_exists='replace'):
    if sql:
        if '.csv' in name:
            name = name.split(".csv")[0]
        write_sql_database(df, name, if_exists)
    if csv:
